    ...
    <div className="min-h-screen bg-background pb-20 pt-[64px] font-serif text-text-primary">
      <header className="fixed top-0 left-0 right-0 h-[64px] bg-background border-b border-border/50 z-50 px-6 flex items-center justify-between transition-all">
          <div className="flex items-center gap-4">
              <button 
                  onClick={() => navigate('/')} 
                  className="p-2 -ml-2 text-text-primary hover:bg-surface-hover transition-colors active:scale-95"
              >
      ...
      <main className="max-w-xl mx-auto px-6 mt-6">
        ...
        <h3 className="font-sans text-xs font-semibold text-text-secondary uppercase tracking-[0.15em] mt-6 mb-2 ml-1">Gemini AI Configuration</h3>
        <div className="bg-surface border border-border p-4">
            
            {/* Model Selection */}
            <div className="mb-4 pb-4 border-b border-border/50">
                <label className="block text-[14px] font-medium mb-3">Preferred AI Model</label>
                <div className="relative">
                    <select
                        value={settings.preferredModel || 'gemini-3-flash-preview'}
                        onChange={(e) => updateSettings({ preferredModel: e.target.value })}
                        className="w-full bg-background border border-border text-text-primary rounded-none p-3 appearance-none focus:outline-none focus:border-text-primary font-serif font-medium text-[14px]"
                    >
            ...
            <div className="flex justify-between items-center mb-3">
                ...
                <button 
                    ...
                    className="font-sans text-[10px] font-semibold uppercase tracking-widest text-text-primary hover:text-text-secondary transition-colors"
                >
            ...
            {isEditingAi ? (
                <div className="animate-fade-in">
                    ...
                    <div className="flex gap-3">
                        ...
                        <button onClick={saveAiKeys} className="flex-1 bg-text-primary text-surface py-3 font-sans text-xs font-semibold uppercase tracking-widest hover:bg-text-primary/90 transition-colors">
                            Save Keys
                        </button>
                    </div>
                </div>
                ...
                <div className="bg-background border border-border p-4 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <div className="w-8 h-8 bg-surface border border-border flex items-center justify-center text-text-primary">
                            <Icon name="sparkles" size="sm" strokeWidth={1.5} />
                        </div>
                        <div>
                            <span className="text-[14px] font-medium">{settings.geminiApiKeys?.length || 0} Keys Active</span>
                            <p className="font-sans text-[10px] font-medium text-text-secondary mt-1">Rotation enabled for stability</p>
                        </div>
                    </div>
                </div>
            )}
        ...
        <h3 className="font-sans text-xs font-semibold text-text-secondary uppercase tracking-[0.15em] mt-6 mb-2 ml-1">Data Management</h3>
        <div className="bg-surface border border-border p-4">
            <div className="flex justify-between items-center mb-3">
                <span className="text-[14px] font-medium">Storage Usage</span>
                <span className="font-sans text-[10px] font-semibold text-text-secondary uppercase tracking-widest">
                    {storageUsage ? `${formatBytes(storageUsage.usage)} / ${formatBytes(storageUsage.quota)}` : '...'}
                </span>
            </div>
            <div className="h-1 bg-border overflow-hidden mb-5">
                <div 
            ...
            <div className="grid grid-cols-2 gap-3">
                <button 
                    ...
                    className="flex flex-col items-center justify-center gap-2 bg-background border border-border text-text-primary px-3 py-4 font-sans text-[10px] font-semibold uppercase tracking-widest active:scale-95 transition-transform hover:bg-surface-hover"
                >
                    <Icon name="download" size="md" strokeWidth={1.5} /> 
                    Export Data
                </button>
                <label className="flex flex-col items-center justify-center gap-2 bg-background border border-border text-text-primary px-3 py-4 font-sans text-[10px] font-semibold uppercase tracking-widest active:scale-95 transition-transform hover:bg-surface-hover cursor-pointer">
            ...
            <div className="flex justify-between items-center mt-5 pt-4 border-t border-border/50">
                <span className="font-sans text-[10px] font-medium text-text-secondary">
                    Last backup: {settings.lastBackupTimestamp ? new Date(settings.lastBackupTimestamp).toLocaleDateString() : 'Never'}
                </span>
                <button onClick={loadAuditLogs} className="font-sans text-[10px] font-semibold uppercase tracking-widest text-text-primary hover:text-text-secondary transition-colors">
                    View Logs
                </button>
            </div>
        </div>

        <h3 className="font-sans text-xs font-semibold text-text-secondary uppercase tracking-[0.15em] mt-6 mb-2 ml-1">Advanced</h3>
        <div className="bg-surface border border-border p-4">
            <div className="flex justify-between items-center mb-3">
                ...
                <div className="mb-4">
                    <div className="flex gap-2 mb-2">
                        <input 
                            ...
                            className="flex-1 bg-background border border-border p-2 text-[13px] font-mono outline-none focus:border-text-primary"
                            placeholder="https://..."
                        />
                        <button onClick={saveUrl} className="bg-text-primary text-surface px-4 font-sans text-[10px] font-semibold uppercase tracking-widest hover:bg-text-primary/90 transition-colors">Save</button>
                    </div>
                </div>
            ) : (
                <div className="bg-background border border-border p-3 text-[12px] text-text-secondary font-mono mb-4 truncate select-all">
            ...
            <button 
                ...
                className="w-full bg-background border border-border text-text-primary py-3 font-sans text-[10px] font-semibold uppercase tracking-widest active:scale-[0.99] transition-transform hover:bg-surface flex items-center justify-center gap-2"
            >
                {isTestingConnection ? 'Testing...' : 'Test Connection'}
            </button>
        </div>

        <h3 className="font-sans text-xs font-semibold text-primary opacity-80 uppercase tracking-[0.15em] mt-6 mb-2 ml-1">Danger Zone</h3>
        <div className="bg-surface border border-primary/20 p-4 mb-10">
            <button 
                onClick={() => setShowClearConfirm(true)}
                className="w-full flex items-center justify-between group"
            >
               <span className="font-sans text-[12px] font-semibold uppercase tracking-widest text-primary">Reset Application</span>
